...
- **Milestone 5 — Prose response loop:** Wire intent → prompt builder → AFM `streamResponse` → bubble updates; passage-scoped invocation only. **M5 follow-up from Mark's M4 device pass (2026-05-01):** make the anchor passage scroll off the top with the chat history once the conversation has at least one user turn — pinning the anchor at the top burns vertical space the user wants for the actual conversation. Pin until first send, then let it scroll with the rest of the content. Validate on iPhone + iPad; the behavior may not need to differ across device sizes since the anchor's role flips from "what is Posey looking at" to "where this conversation started."
