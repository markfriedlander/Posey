...
- **Milestone 2 — Document embedding index: NEXT.** Build chunks at import for all formats (TXT/MD/RTF/DOCX/HTML/EPUB/PDF) per format-parity. Detect language with `NLLanguageRecognizer`, pick the matching `NLEmbedding`, English fallback when no model exists, hash fallback as last resort. Retro-index existing imports on first Ask Posey invocation per document with a brief "Indexing..." state. Persist embeddings as little-endian-packed `BLOB`s; record the chosen embedding model in `embedding_kind` so we can re-index when a model upgrades.
- **Milestone 3 — Two-call intent classifier:** AskPoseyService minimal session lifecycle + `@Generable` `AskPoseyIntent` enum. Unit tests for the classifier; manual on-device probe.
