...
5. 🟡 Empty-state messages on every modal sheet — partial. **TOC sheet** done with two-hardware verification; **Notes/Saved Annotations** already had one; **Audio Export, Preferences, Ask Posey** confirmed not needing one. **Voice picker empty state code drafted but reverted** — can't reach the picker on iPhone via the antenna (NavigationLink inside NavigationStack; TAP verb's registry + accessibility-tree fallback both miss it). Two paths to close it: (a) add an `OPEN_VOICE_PICKER_SHEET` antenna verb (modeled on `OPEN_TOC_SHEET` / `OPEN_PREFERENCES_SHEET`), or (b) add a `TAP_AT_COORD` antenna verb so iPhone tap-by-coordinate works the way the simulator MCP already does. Either unlocks the verification, then re-apply the Voice picker empty-state code.
