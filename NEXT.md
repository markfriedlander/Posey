...
**Tier 3 — Polish**
13. ⏳ Audio export — investigation done (HISTORY 2026-05-07 evening). File IS ~2× faster than live playback at the same rate (6.86s live vs 3.53s file at rate=100% on iPhone, 5-sentence test). NOT a sample-rate or frame-count bug — `AVAudioFile` settings are correct. The acceleration is inside `AVSpeechSynthesizer.write(utterance:)`'s offline-render output. **Awaiting Mark's call** between three options: (1) halve export rate to compensate, (2) time-stretch via `AVAudioEngine`, (3) ship as-is with a "renders ~2× faster than live" label. Re-enable button deferred pending decision.
