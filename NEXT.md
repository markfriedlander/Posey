...
- **Milestone 2 — Document embedding index: IN PROGRESS, polish pending verification.** Service + DB helpers + hooks shipped in `1ba5ea1`. **Three follow-on fixes committed but NOT YET BUILD-VERIFIED** because we hit the 5-hour token limit mid-verification: (a) `StoredDocumentChunk: Equatable` (compile error against `DocumentEmbeddingSearchResult: Equatable`), (b) `nonisolated` annotation on `DocumentEmbeddingIndex`, its supporting types, and `DocumentEmbeddingIndexConfiguration` to fix a MainActor-deinit crash discovered when running the chunking tests on simulator (project default isolation is `MainActor`; XCTest runner deallocs from a non-MainActor thread; deinit hopping via `swift_task_deinitOnExecutorImpl` triggered a TaskLocal scope teardown crash in libmalloc), (c) HISTORY.md / NEXT.md updates.
  - **First action on resume:** sanity-build with `xcodebuild build -scheme Posey -destination 'platform=iOS Simulator,id=D96CD217-387F-4221-A2AA-80B3E737A710' -derivedDataPath /tmp/PoseyM2Fix -quiet`. Concern to watch: `nonisolated struct ...` and `nonisolated enum ...` may not be valid syntax in Swift 5 mode + approachable concurrency — if it doesn't compile, fall back to per-method `nonisolated` or move the structs to a separate file with file-level isolation override. The class-level `nonisolated final class DocumentEmbeddingIndex` should be fine.
  - **Then:** rerun `DocumentEmbeddingChunkingTests`, `DocumentEmbeddingLanguageTests`, `DocumentEmbeddingPersistenceTests` on simulator. They should all pass and (importantly) NOT crash on dealloc. Then ask Mark before doing a device install for the full regression run.
  - Other open follow-up: retro-indexing of pre-existing imports needs to happen on first Ask Posey invocation — the index call is in place but the "Indexing..." UI state belongs in the sheet milestone (4).
