...
- **Milestone 5 — Prose response loop: NEXT.** Wire intent → prompt builder → AFM `streamResponse` → bubble updates; passage-scoped invocation only. Each prose call uses its own `LanguageModelSession` (no transcript reuse across turns; the conversation context is rebuilt per send so independent questions don't bias each other). Streaming snapshots update the assistant bubble's content in place; the typing indicator hides as soon as the first token lands. **M5 follow-up from Mark's M4 device pass (2026-05-01):** make the anchor passage scroll off the top with the chat history once the conversation has at least one user turn — pinning the anchor at the top burns vertical space the user wants for the actual conversation. Pin until first send, then let it scroll with the rest of the content. Validate on iPhone + iPad; the behavior may not need to differ across device sizes since the anchor's role flips from "what is Posey looking at" to "where this conversation started." Acceptance: ask a question with a real anchor on Mark's iPhone, see prose stream in, error path (force AFM unavailable) renders the translated `AskPoseyServiceError` cleanly, cancel-in-flight on dismiss leaves no zombie task.
