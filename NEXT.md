...
- **Milestone 7 — Navigation pattern + auto-save + source attribution + indexing indicator (UI): MOSTLY DONE (2026-05-01).** Source attribution, auto-save to notes, and in-sheet indexing indicator all shipped. Navigation cards (`.search` intent → `@Generable` schema → tap-to-jump) **deferred** — M5's prompt builder routes `.search` through the same prose path with degraded but non-broken behavior; the deeper navigation-card UX needs a deliberate design pass before implementation. Recorded as a polish followup in M9 polish list.
